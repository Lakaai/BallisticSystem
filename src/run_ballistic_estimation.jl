...
function run_filter(t_hist, y_hist, mu0, Σ0)
    ...
    for i = 1:nsteps
        ...
        density = predict(time, density, update_method; sqrt=false) # Form the predicted density p(x[k] ∣ y[k]:y[k-1]) by propagating p(x[k-1] ∣ y[k]:y[k-1]) through the process model 

        # Process the measurement event
        density = update(density, measurement, update_method; sqrt=false) # Compute the filtered density p(x[k] ∣ y[1]:y[k])
