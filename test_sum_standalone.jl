# Standalone test for the sum function
# This includes only the necessary parts from Gaussian.jl

using LinearAlgebra

# Define the Gaussian struct and basic functions needed for testing
struct Gaussian{T}
    mean
    covariance::Matrix{T}
end
...
# The sum function we want to test
...
# Test 4: Error handling for sqrt parameter
println("\nTest 4: Error handling for sqrt=true")
println("-" ^ 30)

try
    Z4 = sum(X1, Y1, sqrt=true)
    println("ERROR: Should have thrown an error!")
catch e
    println("✓ Correctly caught error: $(e.msg)")
end
