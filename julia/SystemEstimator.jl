...
function predict(time::Any, density::Gaussian)
    ...
    density = affineTransform(RK4SDE, density, dt)
    # println("density in predict", density)

    return density
end 

# Affine transform of Gaussian: y = f(x), linearized around mu
function affineTransform(func::Any, density::Gaussian, dt)

    μx = density.mean
    Sx = density.covariance

    # Compute function value and Jacobian using ForwardDiff
    fx = func(μx, dt)
    ...
    return Gaussian(μy, Sy_upper)

end 

    
# Map [x[k]; dw(idxQ)[k]] to x[k+1] using RK4
function RK4SDE(x::Any, dt::Any)
    k1 = dynamics(x)
